...
- `type` — строка `"Simple"` / `"Lazy"` / `"Shield"` / `"Lock"` / `"Parasite"` / `"Anchor"` / `"Bomb"` / `"Splitter"` / `"Virus"` (case-insensitive).
...
- `balance` — **опциональный** объект переопределения баланса для данного уровня. Если поле отсутствует — `LevelConfig.Balance` равен `null`, и все значения берутся из `BalanceConfig.Default` (= `GameBalance`). Если объект присутствует — можно переопределить любое подмножество полей; пропущенные поля также наследуют значения из `GameBalance`. Поля объекта:
  ...
  - `virusBite` (int, default: 2) — урон, наносимый при заражении молекулы Virus (`>= 0`).
  - `splitterChildValue` (int, default: 3) — значение детей, которых спаунит Splitter (`>= 1`).
...
- **`MoleculeType`** — `Simple`, `Lazy`, `Shield`, `Lock`, `Anchor`, `Parasite`, `Bomb`, `Splitter`, `Virus`.
...
- **Тупик (`Stuck`):** живые молекулы остались, но **ни одна не removable** (все защищены
  активным Shield/Lock) — поле не очистить. Терминальный статус (рестарт). Проверяется после
  win/lose и в конструкторе `GameSession`. Солвер и CI-гейт `solver.yml` ловят такие уровни.
...
| **Splitter** | −1 | **до удаления: спаунит 2 новых Simple(`SPLITTER_CHILD_VALUE`, дефолт 3), каждый соединён со всеми живыми соседями сплиттера; дети НЕ соединены друг с другом** | нет | разветвитель — обменивает один узел на два |
| **Virus** | −1 | нет | `VirusPassive` | каждый ход заражает одного живого не-вирусного соседа с наибольшим Value (тай-брейк: наименьший Id): превращает его в Virus и наносит −VIRUS_BITE урона; одно заражение в ход, цепей в тот же ход нет |
...
- `VIRUS_BITE = 2` (урон при заражении; переопределяется через `virusBite` в JSON-блоке `balance`)
- `SPLITTER_CHILD_VALUE = 3` (значение детей Splitter; переопределяется через `splitterChildValue`)

> Teleport из ранних версий GDD **в скоуп не входит**.
...
- **`VirusPassive(bite, dormant=false)`** — пассивка типа Virus. `ModifyDelta` возвращает
  входящий delta без изменений (Virus декрементируется по базовой ставке). `OnPassiveApply`:
  если `dormant=true` → сбрасывает флаг в `false` и завершает (ждёт один ход); иначе —
  ищет среди живых не-вирусных соседей хозяина цель с наибольшим Value (тай-брейк: наименьший
  Id), вызывает на ней `target.MutateTo(Virus, NoAbility, [new VirusPassive(bite, dormant:true)])`
  и `target.ApplyDelta(-bite)`. `Clone` сохраняет оба поля (`_bite`, `_dormant`). Флаг
  `dormant=true` выдаётся свежезаражённым молекулам, чтобы они не разносили инфекцию в тот
  же ход (предотвращает цепи в рамках одного тика `TurnExecutor`). Молекулы, размещённые на
  уровне как Virus, получают `dormant=false` — они заражают с первого же хода.
...
- **Снапшот хранит `MoleculeType` и `IAbility`** (поля `Type` и `Ability` в `MoleculeSnapshot`). Это необходимо для корректного отмены заражения Virus: если молекула была Bomb, заразилась и стала Virus, undo возвращает ей тип `Bomb` и её `BombAbility`. Метод `Molecule.ToSnapshot()` формирует снапшот из собственных полей (включая приватное `Ability`); `TakeSnapshot()` вызывает `molecule.ToSnapshot()` вместо прямого конструктора `MoleculeSnapshot`. `SetFromSnapshot` теперь дополнительно восстанавливает `Type` и `Ability`.
...
- Все 9 типов MVP: **Simple (−1), Lazy (ускоряющийся декремент −1,−2,−3…, кликабельна), Shield (заморожен + блок клика), Lock (блок клика, декремент идёт), Parasite (−число соседей), Anchor (+1 соседям на клик, −2 за ход), Bomb (удаляет всех живых соседей при клике, игнорируя защиту; −1 за ход), Splitter (спаунит 2 Simple(3) с унаследованными связями до своего удаления; −1 за ход), Virus (каждый ход заражает живого не-вирусного соседа с наибольшим Value; тай-брейк наименьший Id; жертва становится Virus и получает −VIRUS_BITE; свежезаражённые dormant 1 ход; −1 за ход)**. `MoleculeFactory` маппит все типы. Мутация типа во время игры реализована через `Molecule.MutateTo(type, ability, passives)`.
- Пассивки: `LazyPassive`, `ShieldPassive`, `LockPassive`, `FreezePassive`, `NoPassive`, `NeighborCountDecrementPassive`
  (паразит), `FlatDecrementPassive` (якорь), `VirusPassive` (вирус, с флагом `dormant`). Абилка `HealNeighborsAbility` (якорь).
...
- **Всего 187 тестов.** (добавлены `SplitterTests.cs` — 6 тестов: базовый спаун, значение детей после хода, события, undo, изолированный сплиттер, сценарий солвера; `VirusTests.cs` — 11 тестов: инфекция, тай-брейк, отсутствие цепи в тот же ход, no-op при только-вирусных соседях, undo отменяет заражение Bomb, undo восстанавливает dormancy, кастомный VirusBite, клонирование VirusPassive × 2, сценарий солвера).
