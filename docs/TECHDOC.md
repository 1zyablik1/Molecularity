...
> ⚠️ **АКТУАЛИЗАЦИЯ (2026-06-17).** Этот ТЗ написан под Unity-проект и его раздел
> «Текущий статус реализации» **устарел** — в репозитории сейчас **чистое .NET-ядро**
> (`dotnet/`), а не Unity-проект. Стратегия: сначала кор-луп на .NET (см.
> [`DOTNETDOC.md`](DOTNETDOC.md)), Unity подключается позже как слой визуализации + мета.
> Канон правил ядра — [`GAME-CORE.md`](GAME-CORE.md). Описанная ниже Unity-архитектура
> (модули, паттерны, оптимизация, монетизация) остаётся **целевой картиной будущего**,
> но «сканер» в ней заменён инвентарём предметов, бесконечный режим отложен, а раскраска
> — основная мета-игра. При противоречии прав `GAME-CORE.md`.
